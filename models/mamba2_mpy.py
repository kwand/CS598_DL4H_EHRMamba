...
transformer blocks for Mamba2 blocks.
"""
...
try:
    ...
    from mambapy.mamba2 import ResidualBlock as Mamba2ResidualBlock
except Exception as exc:
    Mamba2ResidualBlock = None
    Mamba2Config = None
...
class Mamba2Layers(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if Mamba2ResidualBlock is None or Mamba2Config is None:
            raise ImportError(
                "mambapy is required for the Mamba2 model; install mambapy first."
        ...
        self.layers = nn.ModuleList([Mamba2ResidualBlock(self.config) for _ in range(num_layers)])
...
class Mamba2(BaseModel):
    ...
    a seperate stack of Mamba2 blocks. The resulting embedding from each
    stream is concatenated and passed through a linear classification head.
    """
    ...
    def __init__(
        ...
        for feature_key in self.feature_keys:
            self.mamba_layers[feature_key] = Mamba2Layers(
